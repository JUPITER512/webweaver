...
export default authMiddleware({
    ...
    afterAuth(auth,req){
      ...
      const pathWithSeachParams=`${url.pathname}${searchParams.length>0?`?${searchParams}`:""}`
      // if subDomainExists
      const customSubDomain=hostname.get('hosst')?.split(`${process.env.NEXT_PUBLIC_DOMAIN}`).filter(Boolean)[0];
      if(customSubDomain){
